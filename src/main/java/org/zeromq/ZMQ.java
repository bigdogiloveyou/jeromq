 ...
 * </ul>
 *
 * <p>ØMQ轻量级消息传递内核是一个库，该库扩展了标准套接字接口，具有传统上由专用消息传递中间件产品提供的功能。 ØMQ套接字提供了异步消息队列，多种消息传递模式，消息过滤（订阅），对多种传输协议的无缝访问等的抽象。
 * </ p>
 * <p>以下是ØMQ概念的概述，描述了ØMQ如何抽象标准套接字
 * 并提供有关ØMQ库提供的功能的参考手册。</ p>
 * <h2>上下文</ h2>
 * <p>在使用任何ØMQ库函数之前，必须使用{@link ZMQ＃context（int）}创建一个{@link ZMQ#context(int)}.
 * 退出应用程序时，必须使用{@link ZMQ.Context＃close（）}破坏上下文。</ p>
 * <h3>线程安全</ h3>
 * ØMQ上下文是线程安全的，可以根据需要在多个应用程序线程之间共享，
 * 调用方无需任何其他锁定。
 * <br>
 * 个别的ØMQ插座不是螺纹安全的，除非是这种情况
 * 将套接字从一个线程迁移到另一个线程时发出完整的内存屏障的位置。
 * <br>
 * 实际上，这意味着应用程序可以使用* {@link ZMQ.Context＃socket（SocketType）}在一个线程中创建套接字。
 * 然后将其传递给新创建的线程，作为线程初始化的一部分。
 * <h3>多个上下文</ h3>
 * 多个上下文可以在单个应用程序中共存。
 * <br>
 * 因此，应用程序可以直接使用ØMQ，同时可以使用任意数量的附加库
 * 只要遵守上述有关线程安全的准则，或本身使用ØMQ的组件。
 * <h2>消息</ h2>
 * ØMQ消息是在应用程序或同一应用程序的组件之间传递的离散数据单元。
 * ØMQ消息没有内部结构，并且从ØMQ本身的角度来看
 * 它们被认为是不透明的二进制数据。
 * <h2>套接字</ h2>
 * {@link ZMQ.Socket ØMQ sockets} 提供了异步消息队列的抽象，
 * 具有确切的排队语义，具体取决于所使用的套接字类型。
 * <h2>运输</ h2>
 * <p>一个ØMQ套接字可以使用多种不同的底层传输机制。
 * 每种传输机制都适合于特定目的，并且具有其自身的优缺点。</ p>
 * <p>提供了以下传输机制：</ p>
 * <ul>
 * <li>使用TCP的单播传输</ li>
 * <li>本地进程间通信传输</ li>
 * <li>本地进程内（线程间）通信传输</ li>
 * </ ul>
 * <h2>代理</ h2>
 * <p>ØMQ提供代理来创建扇出和扇入拓扑。
 * 代理将前端套接字连接到后端套接字
 * 并在两个套接字之间不透明地切换所有消息。
 * 代理可以选择将所有流量捕获到第三个套接字。</ p>
 * <h2>安全性</ h2>
 * <p>ØMQ插槽可以选择一种安全机制。双方必须使用相同的安全机制。</ p>
 * <p>为IPC和TCP连接提供了以下安全机制：</ p>
 * <ul>
 * <li>无效的安全性</ li>
 * <li>使用用户名和密码的纯文本身份验证</ li>
 * <li>椭圆曲线认证和加密</ li>
 * </ ul>
 */
public class ZMQ
